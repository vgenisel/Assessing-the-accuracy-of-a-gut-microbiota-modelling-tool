...
heatmap_predicted = data.pivot(index="Sample", columns="Analyte", values="Predicted")
# Pivot the data for Measured values
heatmap_measured = data.pivot(index="Sample", columns="Analyte", values="Measured")
# Pivot the data for Difference values
heatmap_difference = data.pivot(index="Sample", columns="Analyte", values="Difference")
...
plt.tight_layout()
fig.savefig('results/heatmap.svg', format='svg', dpi=600)
plt.show()
